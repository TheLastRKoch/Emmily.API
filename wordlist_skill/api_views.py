...
from wordlist_skill.serializers import WordListSerializer, WordSerializer
from rest_framework.permissions import IsAdminUser
from rest_framework.filters import SearchFilter
from wordlist_skill.models import Word, WordList, WordList
...
class WordCreation(CreateAPIView):
    ...
    def create(self,request,*args,**kwargs):
        ...
        if WordList.objects.filter(name = request.data.get("name")).count() != 0:
            raise ValidationError({'WordList name':'Already existe on the DB'})
        return super().create(request,*args,**kwargs)  
